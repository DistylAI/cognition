...
// + data-slot, rounded-md (matches Button), shadow-sm, the v4 focus ring
...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "flex h-9 w-full min-w-0 rounded-md border border-border-default bg-background-default px-3 py-1 text-sm text-text-default shadow-sm transition-colors outline-none",
